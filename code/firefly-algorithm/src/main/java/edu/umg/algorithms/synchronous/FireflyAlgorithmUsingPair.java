...
public class FireflyAlgorithmUsingPair {
            ...
            for (int i = 0; i < populationSize; i++) {
                ...
                if(population[i].getIntensity() > theBestSolution.getIntensity()){
                    ...
                    if(theBestSolution.getIntensity() > finalSolution.getIntensity()){
                        finalSolution = new FireflyUsingPair(theBestSolution, objectiveFunction, minimalize);
                    }
                }
            }
    ...
    private void initializePopulation(){
        ...
        for (int i = 0; i < populationSize; i++) {
            population[i] = new FireflyUsingPair(objectiveFunction, lowerBound, upperBound, minimalize);
        }
    }
    ...
    private double distanceBetweenFireflies(int index1, int index2){
        double result = Math.pow(population[index1].getLocationAt(0) - population[index2].getLocationAt(0), 2);
        result += Math.pow(population[index1].getLocationAt(1) - population[index2].getLocationAt(1), 2);
    ...
    private Pair<Double, Double> computeNewLocation(int index1, int index2){
        ...
        double first = population[index1].getLocationAt(0) + attractiveness * (population[index2].getLocationAt(0) - population[index1].getLocationAt(0)) + computeRandomStep();
        double second = population[index1].getLocationAt(1) + attractiveness * (population[index2].getLocationAt(1) - population[index1].getLocationAt(1)) + computeRandomStep();
    ...
    private Pair<Double, Double> computeNewLocation(){
        double first = population[locationOfTheBestSolution].getLocationAt(0) + computeRandomStep();
        double second = population[locationOfTheBestSolution].getLocationAt(1) + computeRandomStep();
